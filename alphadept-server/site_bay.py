...
class StockPredictor_bay:
    ...
    def predict(self):
        ...
        indices_above_threshold = np.where(prob_predictions > 0.55)[0]

        out = []
        subset['buy'] = False  # Создаем столбец 'buy' и инициализируем его значением False
        ...
        for index in indices_above_threshold:
            ...
            if procent > 0:
                ...
                subset.loc[subset.index == subset.index[index], 'buy'] = True 
        
        return out, subset[['close', 'buy']].to_json()
...
if __name__ == "__main__":
    ...
    if not ticker or not indicator:
        ...
        stock_predictor = StockPredictor_bay(ticker, indicator)
